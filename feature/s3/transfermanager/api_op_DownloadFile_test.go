...
func TestDownloadFileValidatesDirectIOConstraints(t *testing.T) {
	tests := map[string]struct {
		...
		"write alignment": {
			update: func(o *Options) {
				o.PartSizeBytes = 2 * directIOAlignment
				o.WriteChunkSizeBytes = directIOAlignment + 1
			},
			wantErr: "WriteChunkSizeBytes must be a positive multiple",
		},
		"write larger than part": {
			update: func(o *Options) {
				o.PartSizeBytes = directIOAlignment
				o.WriteChunkSizeBytes = 2 * directIOAlignment
			},
			wantErr: "less than or equal",
		},
		"part not divisible by write": {
			update: func(o *Options) {
				o.PartSizeBytes = 3 * directIOAlignment
				o.WriteChunkSizeBytes = 2 * directIOAlignment
			},
			wantErr: "integer multiple",
		},
	}
